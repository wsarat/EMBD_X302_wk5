...
static esp_err_t submit_async_req(httpd_req_t *req, httpd_req_handler_t handler)
{
    ...
    if (xSemaphoreTake(worker_ready_count, ticks) == false) {
        ESP_LOGE(HTTPSERVER_TAG, "No workers are available");
    ...
    if (xQueueSend(async_req_queue, &async_req, pdMS_TO_TICKS(100)) == false) {
        ESP_LOGE(HTTPSERVER_TAG, "worker queue is full");
        httpd_req_async_handler_complete(copy); // cleanup
        return ESP_FAIL;
    }
...
{
    ESP_LOGI(HTTPSERVER_TAG, "uri: /long");
...
{
    ESP_LOGI(HTTPSERVER_TAG, "starting async req task worker");
    ...
    while (true) {
        ...
        if (xQueueReceive(async_req_queue, &async_req, portMAX_DELAY)) {
            ...
            ESP_LOGI(HTTPSERVER_TAG, "invoking %s", async_req.req->uri);
            ...
            if (httpd_req_async_handler_complete(async_req.req) != ESP_OK) {
                ESP_LOGE(HTTPSERVER_TAG, "failed to complete async req");
            }
        }
    }

    ESP_LOGW(HTTPSERVER_TAG, "worker stopped");
    vTaskDelete(NULL);
}
...
static void start_async_req_workers(void)
{
    ...
    if (worker_ready_count == NULL) {
        ESP_LOGE(HTTPSERVER_TAG, "Failed to create workers counting Semaphore");
        return;
    }
    ...
    if (async_req_queue == NULL){
        ESP_LOGE(HTTPSERVER_TAG, "Failed to create async_req_queue");
    ...
    for (int i = 0; i < MAX_ASYNC_REQUESTS; i++) {
        ...
        if (!success) {
            ESP_LOGE(HTTPSERVER_TAG, "Failed to start asyncReqWorker");
            continue;
        }
    }
}
...
{
    ESP_LOGI(HTTPSERVER_TAG, "uri: /quick");
    char s[100];
...
static httpd_handle_t start_webserver(void)
{
    ...
    ESP_LOGI(HTTPSERVER_TAG, "Starting server on port: '%d'", config.server_port);
                               ...
                               int32_t event_id, void* event_data)
{
    ...
    if (*server) {
        ESP_LOGI(HTTPSERVER_TAG, "Stopping webserver");
        if (stop_webserver(*server) == ESP_OK) {
            *server = NULL;
        } else {
            ESP_LOGE(HTTPSERVER_TAG, "Failed to stop http server");
        }
    }
}
                            ...
                            int32_t event_id, void* event_data)
{
    httpd_handle_t* server = (httpd_handle_t*) arg;
    if (*server == NULL) {
        ESP_LOGI(HTTPSERVER_TAG, "Starting webserver");
